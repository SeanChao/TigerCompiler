    ...
    switch (stm->kind) {
        case T_MOVE: {
            ...
            if (src->kind == T_CONST) {
                ...
                AS_instr instr = AS_Oper(String(buf), L(munchExp(dst), NULL),
                                         L(NULL, NULL), NULL);
                emit(instr);
                return;
            }
        ...
        case T_CJUMP: {
            ...
            // cmq s2, s1: s1 - s2
            emit(AS_Oper("cmp `d0, `s0", L(dst, NULL), L(src, NULL), NULL));
            char buf[_LEN];
    ...
    switch (exp->kind) {
        ...
        case T_MEM: {
            ...
            if (mem->kind == T_CONST) {
                ...
                AS_instr instr =
                    AS_Oper(String(buf), L(val, NULL), L(NULL, NULL), NULL);
                emit(instr);
                return val;
